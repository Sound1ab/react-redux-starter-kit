...
After [installing](#installation) the project and its dependencies, all you need to do is start the project!
...
1) If you mistype a property name (`expect(false).to.be.tru`) then this expression evaluates to undefined, the magical getter on the `true` is never run, and so your test silently passed.
2) By default, linters don't understand them and mark them as unused expressions, which can be annoying.

[Dirty Chai](https://github.com/prodatakey/dirty-chai) fixes this by converting these getters into callable functions. This way, if we have a typo in a property name, the value will not be a function and therefore cause the assertion to throw.

```js
// Due to a typo, this silently passes because the getter on `true` is never invoked!
it('should be true', () => {
...
// Much better!
it('should be true', () => {
  expect(false).to.be.tru() // correctly throws
})
```
